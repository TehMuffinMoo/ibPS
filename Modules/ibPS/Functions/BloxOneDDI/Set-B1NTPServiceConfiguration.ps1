  ...
  if ($B1Service) {
    if ($B1Service.count -gt 1) {
      ...
      if ($UseGlobalNTPConfig) {
        ...
        $NewConfigResult = Query-CSP -Method POST -Uri "$(Get-B1CSPUrl)/api/ntp/v1/service/config/$ServiceId" -Data $ConfigSplat | select -ExpandProperty ntp_service -ErrorAction SilentlyContinue
