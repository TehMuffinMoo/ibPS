function Get-B1TideThreatProperty {
    ...
    if ($id) {
      $Results = Query-CSP -Uri "$(Get-B1CspUrl)/tide/api/data/properties/$id$filter" -Method GET | Select -ExpandProperty property -ErrorAction SilentlyContinue -WarningAction SilentlyContinue
    } else {
      $Results = Query-CSP -Uri "$(Get-B1CspUrl)/tide/api/data/properties$filter" -Method GET | Select -ExpandProperty property -ErrorAction SilentlyContinue -WarningAction SilentlyContinue
    }
