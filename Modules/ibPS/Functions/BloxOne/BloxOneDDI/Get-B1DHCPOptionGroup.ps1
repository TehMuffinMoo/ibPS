function Get-B1DHCPOptionGroup {
    ...
    if ($QueryString) {
        Query-CSP -Method GET -Uri "$(Get-B1CSPUrl)/api/ddi/v1/dhcp/option_group$($QueryString)" | Select-Object -ExpandProperty results -ErrorAction SilentlyContinue
    } else {
        Query-CSP -Method GET -Uri "$(Get-B1CSPUrl)/api/ddi/v1/dhcp/option_group" | Select-Object -ExpandProperty results -ErrorAction SilentlyContinue
    }
}