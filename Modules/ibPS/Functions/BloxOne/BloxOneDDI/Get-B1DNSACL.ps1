function Get-B1DNSACL {
    ...
    if ($QueryString) {
        Query-CSP -Method GET -Uri "$(Get-B1CSPUrl)/api/ddi/v1/dns/acl$($QueryString)" | Select-Object -ExpandProperty results -ErrorAction SilentlyContinue
    } else {
        Query-CSP -Method GET -Uri "$(Get-B1CSPUrl)/api/ddi/v1/dns/acl" | Select-Object -ExpandProperty results -ErrorAction SilentlyContinue
    }
}