function Get-B1AddressBlockNextAvailable {
    ...
    process {
        ...
        if ($Parent) {
            Invoke-CSP -Method "GET" -Uri "$(Get-B1CSPUrl)/api/ddi/v1/$($Parent.id)/nextavailableaddressblock?cidr=$CIDRSize&count=$Count" | Select-Object -ExpandProperty results -ErrorAction SilentlyContinue
        } else {
