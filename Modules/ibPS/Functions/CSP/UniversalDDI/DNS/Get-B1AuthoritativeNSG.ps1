function Get-B1AuthoritativeNSG {
    ...
    if ($QueryString) {
        Invoke-CSP -Method GET -Uri "$(Get-B1CSPUrl)/api/ddi/v1/dns/auth_nsg$($QueryString)" | Select-Object -ExpandProperty results -ErrorAction SilentlyContinue
    } else {
        Invoke-CSP -Method GET -Uri "$(Get-B1CSPUrl)/api/ddi/v1/dns/auth_nsg" | Select-Object -ExpandProperty results -ErrorAction SilentlyContinue
    }
}