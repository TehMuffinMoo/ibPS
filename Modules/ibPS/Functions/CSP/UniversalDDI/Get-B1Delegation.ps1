function Get-B1Delegation {
    <#
    .SYNOPSIS
        Queries a list of Delegations from the Universal DDI IPAM
    ...
    #     Use this parameter to filter the list of Delegations with a utilization below the high utilization threshold
    ...
    .EXAMPLE
        PS> Get-B1Delegation -Protocol ip4 -CIDR 24 | ft address,cidr,name,protocol
    ...
    if ($QueryString) {
        Invoke-CSP -Uri "$(Get-B1CSPUrl)/api/ddi/v1/federation/delegation$($QueryString)" -Method GET | Select-Object -ExpandProperty results -ErrorAction SilentlyContinue
    } else {
        Invoke-CSP -Uri "$(Get-B1CSPUrl)/api/ddi/v1/federation/delegation?_limit=$Limit&_offset=$Offset" -Method GET | Select-Object -ExpandProperty results -ErrorAction SilentlyContinue
    }
}