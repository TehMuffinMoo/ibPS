function Get-B1OverlappingBlock {
    <#
    .SYNOPSIS
    ...
    .EXAMPLE
        PS> Get-B1OverlappingBlock -Name "block-1" -Strict
    ...
    if ($QueryString) {
        Invoke-CSP -Uri "$(Get-B1CSPUrl)/api/ddi/v1/federation/overlapping_block$($QueryString)" -Method GET | Select-Object -ExpandProperty results -ErrorAction SilentlyContinue
    } else {
        Invoke-CSP -Uri "$(Get-B1CSPUrl)/api/ddi/v1/federation/overlapping_block?_limit=$Limit&_offset=$Offset" -Method GET | Select-Object -ExpandProperty results -ErrorAction SilentlyContinue
    }
}