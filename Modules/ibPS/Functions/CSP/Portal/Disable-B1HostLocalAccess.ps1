function Disable-B1HostLocalAccess {
    ...
    .EXAMPLE
        PS> Disable-B1HostLocalAccess -Server my-host-1 -UseDefaultCredentials
    ...
    process {
        ...
        if ($LocalAccessCredentials.Username -and $LocalAccessCredentials.Password) {
            ...
            if($PSCmdlet.ShouldProcess("Disable Local Access on: $($Object.display_name)","Disable Local Access on: $($Object.display_name)",$MyInvocation.MyCommand)){
                $Results = Invoke-CSP -Method POST -Uri "$(Get-B1CspUrl)/bootstrap-app/v1/host/$($OPHID)/disable_local_access" -Data $($JSONData)
                if ($Results.Count -eq 1) {
                    Write-Host "Local access disable request successfully sent for: $($Object.display_name)" -ForegroundColor Green
                    if ($Wait) {
                        $Count = 0
                        while ($Count -lt 60) {
                    ...
                    Write-Error "Error. Failed to sent local access disable request for: $($Object.display_name)"
                }
            }
        }
    }
